...
async def label_question(session, question, kbs):
    ...
    if tag_kb_ids:
        ...
        if not all_tags:
            all_tags = await RETRIEVALER.all_tags_in_portion(kb.tenant_id, tag_kb_ids)
...
if __name__ == "__main__":
    ...
    asyncio.run(chunk(sys.argv[1], from_page=0, to_page=10, callback=dummy))